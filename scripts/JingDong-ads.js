...
if (url.includes("functionId=readCustomSurfaceList")) {
    // 底部导航栏   黑暗模式
    if (obj?.data?.result?.modeMap?.dark?.navigationAll.length > 0) {
        ...
        obj.data.result.modeMap.dark.navigationAll = newNavigationList;
    }

    // 底部导航栏   普通模式
    if (obj?.data?.result?.modeMap?.normal?.navigationAll.length > 0) {
        let newNavigationList = [];
        ...
        obj.data.result.modeMap.normal.navigationAll = newNavigationList;
    }
}
